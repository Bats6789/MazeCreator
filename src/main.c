...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "a:ho:qv::", long_opts, &opts_index)) !=
         -1) {
    switch (opt) {
    case 0:
      break;
    ...
    case 'v':
      ...
      if (!optarg) {
        ...
        if (!stepFile) {
          printError("ERROR opening \"steps.step\": %s", strerror(errno));
          return EXIT_FAILURE;
        }
      } else {
  ...
  if (algorithm == growing_tree) {
    if (argv[optind] == NULL || isinteger(argv[optind])) {
      ...
      case randomTree:
        ...
        if (argv[optind + 1] != NULL && !isinteger(argv[optind + 1])) {
      ...
      case oldest_randomTree:
        // if the next item is NULL or a digit, split is set to default
        if (argv[optind + 1] == NULL || (isinteger(argv[optind + 1]) &&
